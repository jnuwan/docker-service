pipeline{
	...
	stages{
		...
		stage('Initialize'){
			steps{
				script{
        				sh 'curl -fsSLO https://get.docker.com/builds/Linux/x86_64/docker-17.04.0-ce.tgz && tar xzvf docker-17.04.0-ce.tgz && sudo mv docker/docker /usr/local/bin && sudo rm -r docker docker-17.04.0-ce.tgz'
				}
			}
		}
